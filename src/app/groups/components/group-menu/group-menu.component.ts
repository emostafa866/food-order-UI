...
export class GroupMenuComponent implements OnInit {
  ...
  acceptUser(userId: number) {
    ...
    this.groupSer.acceptUser(this.accept_user).subscribe(
      (res: any) => {
        console.log('succues');
      },
      (error) => {
  ...
  addToCart(e: any) {
    if ('cart' in localStorage) {
      ...
      let exist = this.foodCart.find(
        (item) => item.foodItem.id == e.foodItem.id
      );
      if (exist) {
